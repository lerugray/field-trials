...
2,664 tests across the seven. Every suite runs with `npm test` and needs nothing but Node.
...
less, not more — the interesting claim is not "a person wrote 2,664 tests", it is "this
...
- **suites** — runs on every push and pull request. A matrix over the seven game
...
The Jacquard Index are under the SIL Open Font License 1.1, and the music tracks in
...
register, and are the one exemption to the everything-is-code-generated rule these
projects otherwise hold to. Every other asset in every game is generated by that game's
own code.
