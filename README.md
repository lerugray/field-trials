...
| [MATERIAL BREACH](games/material-breach) | You run the dungeon. Adventurers are an occupational incident, filed with a root-cause analysis and a corrective-action owner. | 208 | **Release gate** — 9-step run 2026-08-18; a save-brick blocker and a phantom notice fix caught and closed in-gate; 205/205. Hotfixed 2026-08-19: the examiner's Q3 finding (a full refund implemented and shipped, but unreachable from any player action) wired to a live Withdraw control on the action bar; 208/208. **Released 2026-08-26**: full gate re-run 2026-08-23 re-certified all step-7 findings fixed; battery 208/208 re-run by the shipping session; live artifact hash-verified; step-9 operator waiver recorded. Hotfixed 2026-08-26 eve: red-with-shadow display text illegible on the live build (operator-reported); worst pair measured 1.77:1 -> 4.40:1, before/after looker-verified, 296/296 re-run at ship. | [play](https://lerugray.github.io/field-trials/material-breach/) |
| [POPINJAY](games/popinjay) | A single-screen arcade roguelite on Pang's bouncing-balloon core — a 1900s World's Fair sharpshooter, wire-as-wall commitment, hand-tinted poster idiom. | 296 | **Release gate** — 9-step run 2026-08-18; a fused title-footer caught at step 2 and a false climb certification exposed, rebuilt, and independently looker-verified; 287/287. Hotfixed 2026-08-19: title confirm-dialog overlay collision and a real walk cycle (was hat-bob only), both operator-reported, both independently re-verified; 291/291. **Released 2026-08-26**: 2026-08-23 re-run treated all prior greens as hearsay and re-executed every claim; battery 291/291 re-run by the shipping session; live artifact hash-verified; step-9 operator waiver recorded. | [play](https://lerugray.github.io/field-trials/popinjay/) |
...
3,759 tests across the twelve on the shelf; ADVERSARY's 300 keep running in CI while it is
...
Ray's pixel-art library (Willibab collection, CC BY, attribution in credits); MATERIAL
BREACH's cast is from the same collection's NPC Pack (CC BY, ATTRIBUTION.md) with type
from the Not Jam Font Pack (CC0); and
...
One more note on music, and on creative text generally: where a game warrants the
designer's own creative imprint rather than a builder's generic register, the direction —
music prompts, register briefs, occasionally the words themselves — is authored through
**Weiss**, a voice model trained on Ray's own writing corpus. The render may be a
machine's; the vocabulary it is held to is his.
