...
Eight browser games. Each one is a complete, self-contained HTML file with no runtime
...
| [ADVERSARY](games/adversary) | Six-stage action-RPG with a Souls death loop | 300 | **FIX-FIRST**, followed by two documented fix rounds | [play](https://lerugray.github.io/field-trials/adversary/) |

2,964 tests across the eight. Every suite runs with `npm test` and needs nothing but Node.
...
given a contract and held to it. Seven of the eight seeds are published here verbatim;
...
- **suites** — runs on every push and pull request. A matrix over the eight game
...
Ray's pixel-art library (Willibab collection, CC BY, attribution in credits). Every other
