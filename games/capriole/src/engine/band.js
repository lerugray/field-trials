...
//   fm     (t, freq, dur, o)  two-operator FM toy voice — glassy carnival lead
...
export function createBand({
  ...
  // A compact two-operator FM voice. WebAudio has no dedicated FM oscillator, so the
  // modulator is routed into the carrier frequency AudioParam. The integer-ish ratio and
  // fast modulation-index decay give it the bright, slightly metallic toy-keyboard edge
  // CAPRIOLE's score needs without samples or a second audio path.
  function fm(t, freq, dur, o = {}) {
    const o1 = ctx.createGain(); o1.gain.value = 1; o1.connect(dry());
    send(o1, o.verb === undefined ? 0.38 : o.verb);
    const f = ctx.createBiquadFilter();
    f.type = 'lowpass';
    try { f.frequency.value = o.cut || 4200; f.Q.value = o.q === undefined ? 1.1 : o.q; } catch (_) { /* stub */ }
    f.connect(o1);
    ...
    const index = o.index === undefined ? 1.8 : o.index;
    try {
      carrier.frequency.value = freq;
      mod.frequency.value = freq * ratio;
      modGain.gain.setValueAtTime(freq * index, t);
      modGain.gain.exponentialRampToValueAtTime(Math.max(EPS, freq * 0.04), t + Math.max(0.05, dur * 0.75));
    } catch (_) { /* stub */ }
    mod.connect(modGain); modGain.connect(carrier.frequency);

    const g = ctx.createGain();
    const tail = env(g, t, {
      a: o.a === undefined ? 0.006 : o.a,
      d: o.d === undefined ? Math.max(0.05, dur * 0.35) : o.d,
      s: o.s === undefined ? 0.42 : o.s,
      r: o.r === undefined ? 0.32 : o.r,
      peak: o.vol === undefined ? 0.065 : o.vol,
      dur,
    });
    carrier.connect(g); g.connect(f);
    carrier.start(t); mod.start(t);
    carrier.stop(t + tail + 0.3); mod.stop(t + tail + 0.3);
  }
  ...
  const voices = { pad, drone, bell, pluck, bass, lead, fm, air, kick, snare, hat };
