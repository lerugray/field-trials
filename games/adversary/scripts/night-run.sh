#!/usr/bin/env bash
# ADVERSARY — bounded autonomous build session (home-PC WSL).
# Usage: scripts/night-run.sh [hours]   (default 6h hard cap)
# Same Ray-approved harness pattern as chapel-perilous/oddseedz/super-sopranos-kart.
set -u
REPO="/mnt/c/Users/rweis/OneDrive/Documents/adversary-game"
HOURS="${1:-6}"
CAP=$(( HOURS * 3600 ))
cd "$REPO" || exit 1
