...
test('m1 world graph validates and every scene is reachable', () => {
  const { graph, startScene } = buildM1World();
...
test('the embedded toy case still passes the full solver battery (with documents)', () => {
...
test('every fact is acquirable from some hotspot; every statement via dialogue', () => {
  ...
  // statements surface through dialogue revealStatement effects
...
test('hotspot fact/suspect/document refs all resolve', () => {
  const { graph, caseData, documents } = buildM1World();
  for (const id of graph.ids()) for (const h of graph.get(id).hotspots) {
    if (!h.meta) continue;
...
test('no em dashes in player-facing world/document text', () => {
  const { graph, documents } = buildM1World();
  const strings = [];
