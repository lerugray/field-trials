...
// The licensed TYPE FACE (Not Jam "Undead Pixel 8", CC0). Unlike the art packs
// it does not become a base64 data URI: scripts/extract-font.mjs bakes the
// sheet into src/font-data.js, which the module concatenation below inlines
// like any other source file — so the face is present on the first painted
// frame and pixelTextWidth() stays exact and synchronous under Node for the
// text/layout gates. This function is the equivalent of inlineArt()'s
// missing-asset throw: no face, no build.
const FONT_SRC_DIR = resolve(ROOT, 'materials/fonts/not-jam-undead-pixel-8');
const FONT_DATA_PATH = resolve(SRC, 'font-data.js');

function verifyFontEmbedded() {
  for (const p of [resolve(FONT_SRC_DIR, 'Glyphs.png'), resolve(FONT_SRC_DIR, 'Licence.txt'), FONT_DATA_PATH]) {
    if (!existsSync(p)) throw new Error(`type face missing: ${relative(ROOT, p)} (run: node scripts/extract-font.mjs)`);
  }
  const table = readFileSync(FONT_DATA_PATH, 'utf8');
  if (!/GENERATED by scripts\/extract-font\.mjs/.test(table)) {
    throw new Error('src/font-data.js lost its provenance banner — regenerate it, do not hand-edit');
  }
  if (!/FONT_LICENSE = 'CC0'/.test(table)) throw new Error('src/font-data.js does not declare its CC0 licence');
  const glyphs = (table.match(/^ {2}"(?:[^"\\]|\\.)+": \[/gm) || []).length;
  if (glyphs < 95) throw new Error(`src/font-data.js carries only ${glyphs} glyphs — the sheet slice is incomplete`);
  const attribution = readFileSync(ATTRIBUTION_PATH, 'utf8');
  if (!/Not Jam/.test(attribution)) throw new Error('ATTRIBUTION.md does not credit the Not Jam type face');
  return { glyphs, bytes: Buffer.byteLength(table, 'utf8') };
}
...
// A wrapped import ("import {\n  a, b,\n} from '...'") is NOT matched by the
// line-wise rule above, so it would survive into the bundle and break the
// single-file build at boot with "Cannot use import statement outside a
// module". Fail LOUDLY at build time instead of shipping that.
const IMPORT_START_RE = /^\s*import\b/;
...
function readModule(absPath) {
  ...
  for (const line of lines) {
    ...
    if (IMPORT_START_RE.test(line)) {
      throw new Error(
        `${relative(ROOT, absPath)}: import statement is not on ONE line — the bundler strips imports line-wise.\n  ${line.trim()}`,
      );
    }
...
`;
  ...
  console.log(`[build] wrote ${relative(ROOT, OUT)} (${kb} KB, ${modules.length} modules, ${ART_KEYS.length} battlers + ${TAROT_KEYS.length} tarot ~${artKb} KB art, `
    + `type face ${face.glyphs} glyphs ~${(face.bytes / 1024).toFixed(0)} KB)`);
}
