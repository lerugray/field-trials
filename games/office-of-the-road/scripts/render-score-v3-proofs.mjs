// render-score-v3-proofs.mjs — listen set for OOR score V3.
// Full-cycle WAV + per-section WAVs per track into proof/score-v3-YYYYMMDD/.
// Full cycles are assembled by concatenating section renders (keeps Offline
// buffers tractable at minute-scale lengths).
...
const DATE = '20260813';
const OUT_DIR = resolve(ROOT, 'proof', `score-v3-${DATE}`);
const SEED = 20260813;
...
window.renderTrackSlice = async function(name, durationSec, seed, startStep) {
  ...
  // Return planar float channels as base64 float32 — Node assembles the WAV.
...
async function renderSlice(page, name, durationSec, startStep) {
  ...
  // Float32Array views the underlying ArrayBuffer; copy into a plain Float32Array.
...
async function main() {
  ...
  // Large section buffers travel as base64; give the page room.
  ...
  for (const name of TRACK_NAMES) {
    ...
    for (let s = 0; s < SECTION_COUNT; s++) {
      ...
      writeFileSync(resolve(OUT_DIR, filename), wav);
      written.push(filename);
      console.log(`[render] wrote ${filename} (${(wav.length / 1024).toFixed(1)} KB)`);
    }
    ...
    const fullName = `oor-${name}-fullcycle-${DATE}.wav`;
