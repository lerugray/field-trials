// shoot-m13cb — full-colour recognizability grid + M12 stage-center grid for the
...
`;
  ...
  const tmp = resolve(ROOT, 'dist/_m13cb-rec.html');
  await writeFile(tmp, html, 'utf8');
  await page.goto(pathToFileURL(tmp).href);
  await page.waitForFunction('window.__done === true', { timeout: 5000 });
  const el = await page.$('#grid');
  await el.screenshot({ path: resolve(OUT, `${DATE}-m13cb-recognizability-grid.png`) });
...
`;
  ...
  const tmp = resolve(ROOT, 'dist/_m13cb-stage.html');
  await writeFile(tmp, html, 'utf8');
  await page.goto(pathToFileURL(tmp).href);
  await page.waitForFunction('window.__done === true', { timeout: 5000 });
  const el = await page.$('#grid');
  await el.screenshot({ path: resolve(OUT, `${DATE}-m13cb-stage-center-grid.png`) });
  await rm(tmp, { force: true });
}
...
async function stageWindow(page) {
  ...
  await stage.screenshot({ path: resolve(OUT, `${DATE}-m13cb-m12-stage-window.png`) });
}
...
async function run() {
  ...
  console.log('m13cb recognizability grid written');
  await stageCenterGrid(gridPage);
  console.log('m13cb stage-center grid written');
  ...
  console.log('m13cb app stage window written');
  await appPage.close();
