// shoot-m13b — a full-colour recognizability grid of the roster so the operator
// can verify the M13b cue pass makes every audited species read label-blind.
...
async function run() {
  ...
  const tmp = resolve(ROOT, 'dist/_m13b.html');
  ...
  await el.screenshot({ path: resolve(OUT, `${DATE}-m13b-recognizability-grid.png`) });
  console.log('m13b recognizability grid written; console errors:', errors.length);
