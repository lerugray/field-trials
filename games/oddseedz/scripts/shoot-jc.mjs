// shoot-jc — Joe Camel final-round proof grids: full-colour recognizability +
// stage-center grids for the mascot-likeness pivot.
...
`;
  ...
  const tmp = resolve(ROOT, 'dist/_jc-rec.html');
  await writeFile(tmp, html, 'utf8');
  await page.goto(pathToFileURL(tmp).href);
  await page.waitForFunction('window.__done === true', { timeout: 5000 });
  const el = await page.$('#grid');
  await el.screenshot({ path: resolve(OUT, `${DATE}-jc-recognizability-grid.png`) });
...
`;
  ...
  const tmp = resolve(ROOT, 'dist/_jc-stage.html');
  await writeFile(tmp, html, 'utf8');
  await page.goto(pathToFileURL(tmp).href);
  await page.waitForFunction('window.__done === true', { timeout: 5000 });
  const el = await page.$('#grid');
  await el.screenshot({ path: resolve(OUT, `${DATE}-jc-stage-center-grid.png`) });
  await rm(tmp, { force: true });
}
...
async function run() {
  ...
  console.log('jc recognizability grid written');
  await stageCenterGrid(gridPage);
  console.log('jc stage-center grid written');
