#!/usr/bin/env bash
# POPINJAY — bounded autonomous build session (home-PC WSL).
# Same harness pattern as chapel-perilous/office-of-the-road/capriole (Ray-ratified
# gauntlet convention; Ray approved this gauntlet with "same parameters" 2026-08-09).
# Usage: scripts/night-run.sh [hours]   (default 6h hard cap)
set -u
REPO="/mnt/c/Users/rweis/OneDrive/Documents/popinjay"
HOURS="${1:-6}"
CAP=$(( HOURS * 3600 ))
cd "$REPO" || exit 1
