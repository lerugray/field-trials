// mfix-capture.mjs — F3 M/X looker verification frames (title + HUD body copy).
...
const OUT = resolve(ROOT, 'docs/proofs/fixround-looker-20260812');
...
async function main() {
  ...
  const titlePath = await shot(page, 'mfix-title_1440x900@1x', ts);

  await page.evaluate(() => window.POPINJAY.startStageAt(1, 1));
  await page.waitForTimeout(600);
  const hudPath = await shot(page, 'mfix-hud-composure_1440x900@1x', ts);
  ...
  await browser.close();
  console.log(JSON.stringify({ titlePath, hudPath, errors: diag.errors, ts }, null, 2));
}
