...
//
// PERFORMANCE PASS. A fairground band is people, not a sequencer: the cornet leans a
// hair late, a strummed chord is not a block chord, and no two hits land at exactly
// the same weight. Drag and lift both come from the BAND'S OWN seeded PRNG (s.rand —
// a hash of the step count and the band seed), so a seeded world always plays the
// same performance. Drag is ONE-SIDED — a note may lean late, never early, so nothing
// can be scheduled into the past. The rhythm section (tuba, snare, hat) stays dead on
// the grid; only the melodic and chordal voices breathe.
//
...
// The performance pass. `drag` is one-sided lateness in seconds; `lift` is a velocity
// multiplier centred on 1. Both are deterministic functions of the step and the band
// seed via s.rand — never a wall clock, never Math.random.
const RND = (s, k) => (s && typeof s.rand === 'function' ? s.rand(k) : 0.5);
const drag = (s, k, secs) => RND(s, k) * secs;
const lift = (s, k, amt) => 1 + (RND(s, k) - 0.5) * amt;
// A strummed chord, not a block chord: one drag for the hand, a small spread for the
// strings. `r` is the release — chordal parts ring on rather than being chopped off.
function strum(s, v, t, notes, dur, o, k = 3, spread = 0.0035) {
  const d = drag(s, k, 0.005);
  for (let j = 0; j < notes.length; j++) v(t + d + j * spread, notes[j], dur, o);
}
...
export const TRACKS = {
  ...
  title: {
    ...
    step(i, t, s) {
      ...
      if (strain === 0) {
        ...
        if (i === 0 || i === 8) s.v.bass(t, i === 0 ? OOM.C2 : OOM.G2, 0.22, { vol: 0.5 * lift(s, 1, 0.06), cut: 700 });
        if (i === 4 || i === 12) strum(s, s.v.pluck, t, prog, 0.2, { vol: 0.16 * lift(s, 4, 0.1), wave: 'triangle', r: 0.55 });
        if (i === 2 || i === 10) s.v.lead(t + drag(s, 5, 0.009), CORNET[(s.n >> 1) % CORNET.length], 0.3, { vol: 0.14 * lift(s, 6, 0.1), wave: 'triangle', r: 0.5 });
        ...
        if (i === 0) s.v.bass(t, root, 0.26, { vol: 0.46 * lift(s, 1, 0.06), cut: 660 });
        if (i === 10) s.v.bass(t, root, 0.2, { vol: 0.34 * lift(s, 2, 0.08), cut: 640 });
        if (i === 4 || i === 12) strum(s, s.v.pluck, t, prog, 0.24, { vol: 0.15 * lift(s, 4, 0.1), wave: 'triangle', r: 0.6 });
        if (i === 2 || i === 6 || i === 12) s.v.lead(t + drag(s, 5, 0.009), TRIO[((bar << 2) + (i >> 2)) % TRIO.length], 0.34, { vol: 0.13 * lift(s, 6, 0.1), wave: 'triangle', r: 0.55 });
  ...
  stage: {
    ...
    step(i, t, s) {
      ...
      if (i % 4 === 0) s.v.hat(t, { vol: 0.05 * lift(s, 7, 0.12) });
      if (i === 8) s.v.snare(t, { vol: 0.09 * lift(s, 4, 0.1) });                         // backbeat
      if (strain === 0) {
        const prog = [CHORDS.I, CHORDS.IV, CHORDS.V, CHORDS.I][bar];
        const root = [OOM.C2, OOM.F2, OOM.G2, OOM.C2][bar];
        if (i === 0 || i === 8) s.v.bass(t, root, 0.2, { vol: 0.5 * lift(s, 1, 0.06), cut: 780 });          // OOM
        if (i === 4 || i === 12) strum(s, s.v.pluck, t, prog, 0.16, { vol: 0.17 * lift(s, 4, 0.1), wave: 'triangle', r: 0.42 }, 2); // PAH
        if (i === 2 || i === 6 || i === 10 || i === 14) s.v.lead(t + drag(s, 5, 0.008), CORNET[(s.n >> 1) % CORNET.length], 0.22, { vol: 0.15 * lift(s, 6, 0.1), wave: 'square' });
      } else {
        ...
        if (i === 0 || i === 8) s.v.bass(t, root, 0.2, { vol: 0.5 * lift(s, 1, 0.06), cut: 780 });
        if (i === 4 || i === 12) strum(s, s.v.pluck, t, prog, 0.18, { vol: 0.17 * lift(s, 4, 0.1), wave: 'triangle', r: 0.46 }, 2);
        if (i === 14) s.v.snare(t, { vol: 0.07 * lift(s, 8, 0.1) });
        if (RAG_STEPS[i] !== undefined) s.v.lead(t + drag(s, 5, 0.008), RAG[(bar * 6 + RAG_STEPS[i]) % RAG.length], 0.2, { vol: 0.15 * lift(s, 6, 0.1), wave: 'square' });
      }
    },
  },
  ...
  waltz: {
    ...
    step(i, t, s) {
      ...
      if (strain === 0) {
        ...
        if (i === 0) s.v.bass(t, OOM.C2, 0.28, { vol: 0.4 * lift(s, 1, 0.06), cut: 620 });
        if (i === 4 || i === 8) strum(s, s.v.pluck, t, prog, 0.24, { vol: 0.12 * lift(s, 4, 0.1), wave: 'triangle', r: 0.7 }, 3, 0.005);
        if (i === 0 || i === 6) s.v.lead(t + drag(s, 5, 0.01), WALTZ_MEL[(s.n / 6 | 0) % WALTZ_MEL.length], 0.42, { vol: 0.12 * lift(s, 6, 0.1), wave: 'triangle', r: 0.8 });
      } else {
        ...
        if (i === 0) s.v.bass(t, root, 0.3, { vol: 0.38 * lift(s, 1, 0.06), cut: 600 });
        if (i === 4 || i === 8) strum(s, s.v.pluck, t, prog, 0.26, { vol: 0.115 * lift(s, 4, 0.1), wave: 'triangle', r: 0.75 }, 3, 0.005);
        if (i === 0 || i === 4 || i === 8) s.v.lead(t + drag(s, 5, 0.01), WALTZ_B[(bar * 3 + (i >> 2)) % WALTZ_B.length], 0.36, { vol: 0.115 * lift(s, 6, 0.1), wave: 'triangle', r: 0.8 });
  ...
  panic: {
    ...
    step(i, t, s) {
      ...
      if (i % 4 === 0) s.v.bass(t, root, 0.14, { vol: 0.46 * lift(s, 1, 0.05), cut: 900 });   // driving beat
      if (heat > 0.4 && i % 4 === 2) s.v.bass(t, root * 2, 0.1, { vol: 0.3, cut: 1100 });     // offbeat push
      if (i % 4 === 0) s.v.snare(t, { vol: 0.11 * lift(s, 4, 0.08) });
      if (heat > 0.7 && i % 2 === 1) s.v.hat(t, { vol: 0.05 });
      if (i % 8 === 0) strum(s, s.v.pluck, t, prog, 0.12, { vol: 0.14 * lift(s, 3, 0.08), wave: 'square', r: 0.3 }, 2, 0.002);
      if (strain === 0) {
        if (i === 4 || i === 12) s.v.lead(t + drag(s, 5, 0.005), CORNET[(s.n >> 1) % CORNET.length] * (heat > 0.6 ? 2 : 1), 0.16, { vol: 0.14 + heat * 0.05, wave: 'square' });
      } else {
        if (i % 4 === 2) s.v.lead(t + drag(s, 5, 0.005), GALOP_B[(bar * 4 + (i >> 2)) % GALOP_B.length] * (heat > 0.6 ? 2 : 1), 0.13, { vol: 0.13 + heat * 0.05, wave: 'square' });
        if (i === 14) s.v.snare(t, { vol: 0.08 * lift(s, 8, 0.08) }); // the galop's lift
      }
    },
  },
};
